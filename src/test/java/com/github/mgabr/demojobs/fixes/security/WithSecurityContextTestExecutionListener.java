package com.github.mgabr.demojobs.fixes.security;
...
public class WithSecurityContextTestExecutionListener extends AbstractTestExecutionListener {
    ...
    public void beforeTestMethod(TestContext testContext) throws Exception {
        ...
        if (testSecurityContext.getTestExecutionEvent() == TestExecutionEvent.TEST_METHOD) {
            TestSecurityContextHolder.setContext(testSecurityContext.getSecurityContext());
        } else {
            testContext.setAttribute(SECURITY_CONTEXT_ATTR_NAME, testSecurityContext);
        }
    }
    ...
    public void beforeTestExecution(TestContext testContext) {
        TestSecurityContext testSecurityContext = (TestSecurityContext)
                testContext.removeAttribute(SECURITY_CONTEXT_ATTR_NAME);
        if(testSecurityContext != null) {
            TestSecurityContextHolder.setContext(testSecurityContext.getSecurityContext());
        }
    }
    ...
    private TestSecurityContext createTestSecurityContext(AnnotatedElement annotated,
        ...
        try {
            return new TestSecurityContext(factory, annotation, initialize);
        }
    ...
    static class TestSecurityContext {
        private final WithSecurityContextFactory<Annotation> securityContextFactory;
        private final Annotation annotation;
        private final TestExecutionEvent testExecutionEvent;

        TestSecurityContext(
                WithSecurityContextFactory<Annotation> securityContextFactory,
        ...
        public SecurityContext getSecurityContext() {
            return this.securityContextFactory.createSecurityContext(annotation);
        }
